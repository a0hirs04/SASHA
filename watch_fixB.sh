#!/bin/bash
# ─────────────────────────────────────────────────────────────────────
#  watch_fixB.sh — Monitor RC1 + RC2 Fix-B jobs in parallel
#  Usage:  bash watch_fixB.sh [poll_interval_seconds]
# ─────────────────────────────────────────────────────────────────────
set -u
...
RC2_JOB=9533
RC1_JOB=9534
RC2_DIR="/home/a0hirs04/PROJECT-NORTHSTAR/build/rc2_fixB_seed42/replicate_01_seed42/output"
RC1_DIR="/home/a0hirs04/PROJECT-NORTHSTAR/build/rc1_fixB_seed42/replicate_01_seed42/output"
...
# ── Initial node check ────────────────────────────────────────────
echo -e "\n${CYN}═══════════════════════════════════════════════════════════${RST}"
echo -e "${CYN}  Fix-B Job Monitor — RC1 ($RC1_JOB) + RC2 ($RC2_JOB)${RST}"
echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
...
while [[ $rc1_done -eq 0 || $rc2_done -eq 0 ]]; do
    ...
    echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
    echo -e "${CYN}  Fix-B Job Monitor          $(date '+%H:%M:%S')${RST}"
    echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
...
echo ""
echo "  To evaluate:"
echo "    python3 evaluate_rc2.py   (update OUT_DIR to rc2_fixB_seed42)"
echo "    python3 evaluate_rc1.py   (update work_dir to rc1_fixB_seed42)"
echo ""
