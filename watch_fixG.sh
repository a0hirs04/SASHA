#!/bin/bash
# ─────────────────────────────────────────────────────────────────────
#  watch_fixG.sh — Monitor RC1 + RC2 Fix-G jobs in parallel
#  Usage:
#    bash watch_fixG.sh [poll_seconds] [rc2_jobid] [rc1_jobid]
#
#  Examples:
#    bash watch_fixG.sh
#    bash watch_fixG.sh 20 9794 9795
# ─────────────────────────────────────────────────────────────────────
set -u
...
RC2_DIR="/home/a0hirs04/PROJECT-NORTHSTAR/build/rc2_fixG_seed42/replicate_01_seed42/output"
RC1_DIR="/home/a0hirs04/PROJECT-NORTHSTAR/build/rc1_fixG_seed42/replicate_01_seed42/output"
...
if [[ -z "$RC2_JOB" ]]; then
    RC2_JOB=$(autodetect_jobid_by_name "rc2_fixG_s42")
fi
if [[ -z "$RC1_JOB" ]]; then
    RC1_JOB=$(autodetect_jobid_by_name "rc1_fixG_s42")
fi

# ── Header ─────────────────────────────────────────────────────────
echo -e "\n${CYN}═══════════════════════════════════════════════════════════${RST}"
echo -e "${CYN}  Fix-G Job Monitor — RC1 (${RC1_JOB:-N/A}) + RC2 (${RC2_JOB:-N/A})${RST}"
echo -e "${CYN}  crowding_base_pressure=0.3, contact_inhibition_threshold=0.8${RST}"
echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
...
if [[ -z "$RC1_JOB" || -z "$RC2_JOB" ]]; then
    ...
    echo -e "      bash watch_fixG.sh ${POLL} <rc2_jobid> <rc1_jobid>"
fi
...
while [[ $rc1_done -eq 0 || $rc2_done -eq 0 ]]; do
    ...
    echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
    echo -e "${CYN}  Fix-G Job Monitor          $(date '+%H:%M:%S')${RST}"
    echo -e "${CYN}  crowding_base_pressure=0.3  contact_inhibition_threshold=0.8${RST}"
    echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
    ...
    echo -e "\n  ${CYN}Parameters: Fix-G crowding_base=0.3, save_interval=360${RST}"
...
echo ""
echo "  To evaluate:"
echo "    python3 evaluate_rc2.py   (OUT_DIR -> rc2_fixG_seed42)"
echo "    python3 evaluate_rc1.py   (work_dir -> rc1_fixG_seed42)"
echo ""
