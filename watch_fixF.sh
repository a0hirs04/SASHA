#!/bin/bash
# ─────────────────────────────────────────────────────────────────────
#  watch_fixF.sh — Monitor RC1 + RC2 Fix-F jobs in parallel
#  Usage:
#    bash watch_fixF.sh [poll_seconds] [rc2_jobid] [rc1_jobid]
#
#  Examples:
#    bash watch_fixF.sh
#    bash watch_fixF.sh 20
#    bash watch_fixF.sh 20 9821 9822
#
#  If job IDs are not provided, the script attempts auto-detection by job name:
#    rc2_fixF_s42, rc1_fixF_s42
# ─────────────────────────────────────────────────────────────────────
set -u
...
RC2_DIR="/home/a0hirs04/PROJECT-NORTHSTAR/build/rc2_fixF_seed42/replicate_01_seed42/output"
RC1_DIR="/home/a0hirs04/PROJECT-NORTHSTAR/build/rc1_fixF_seed42/replicate_01_seed42/output"
...
if [[ -z "$RC2_JOB" ]]; then
    RC2_JOB=$(autodetect_jobid_by_name "rc2_fixF_s42")
fi
if [[ -z "$RC1_JOB" ]]; then
    RC1_JOB=$(autodetect_jobid_by_name "rc1_fixF_s42")
fi

# ── Header ─────────────────────────────────────────────────────────
echo -e "\n${CYN}═══════════════════════════════════════════════════════════${RST}"
echo -e "${CYN}  Fix-F Job Monitor — RC1 (${RC1_JOB:-N/A}) + RC2 (${RC2_JOB:-N/A})${RST}"
echo -e "${CYN}  contact_inhibition_threshold=0.8 + Fix-F settings${RST}"
echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"

if [[ -z "$RC1_JOB" || -z "$RC2_JOB" ]]; then
    echo -e "${YLW}Note:${RST} Could not auto-detect one or both job IDs from sacct."
    echo -e "      You can still monitor file progress; or pass IDs explicitly:"
    echo -e "      bash watch_fixF.sh ${POLL} <rc2_jobid> <rc1_jobid>"
fi
...
while [[ $rc1_done -eq 0 || $rc2_done -eq 0 ]]; do
    ...
    echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
    echo -e "${CYN}  Fix-F Job Monitor          $(date '+%H:%M:%S')${RST}"
    echo -e "${CYN}═══════════════════════════════════════════════════════════${RST}"
    ...
    echo -e "\n  ${CYN}Parameters: Fix-F (check run config), save_interval=360${RST}"

    # If no IDs, stop once snapshot counts reach expected
...
echo ""
echo "  To evaluate:"
echo "    python3 evaluate_rc2.py   (OUT_DIR -> rc2_fixF_seed42)"
echo "    python3 evaluate_rc1.py   (work_dir -> rc1_fixF_seed42)"
echo ""
